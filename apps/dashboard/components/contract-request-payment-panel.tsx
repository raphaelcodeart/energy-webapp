              ...
              className={`relative w-full text-left p-4 rounded-xl border transition ${
                !option.available
                  ...
                  : isSelected
                    ...
                    : discounted
                      ? "cursor-pointer bg-emerald-500/5 border-emerald-500/40 hover:bg-emerald-500/10"
                      : "cursor-pointer bg-white/5 light:bg-slate-900/5 border-white/10 light:border-slate-300 hover:bg-white/10"
              }`}
            >
              {discounted && (
                <span className="absolute -top-2.5 right-3 px-2.5 py-0.5 rounded-full text-[11px] font-extrabold bg-gradient-to-r from-emerald-500 to-emerald-400 text-white shadow-lg shadow-emerald-500/30">
              ...
              <div className="flex items-start justify-between gap-3">
                <div className="min-w-0">
                  <p className="text-sm font-semibold text-white light:text-slate-900">
                    {option.label}
                    {discounted && <span className="ml-2 text-emerald-400">sconto {option.discount_percentage}%</span>}
                  </p>
                ...
                <div className="text-right shrink-0">
                  {discounted && (
                    <p className="text-xs text-slate-500 line-through tabular-nums">{euro(option.list_total_cents)}</p>
                  )}
                  <p className={`text-lg font-bold tabular-nums ${discounted ? "text-emerald-400" : "text-orange-400"}`}>{euro(option.instalment_cents)}</p>
                  {option.instalments > 1 && <p className="text-[10px] text-slate-500">al mese × {option.instalments}</p>}
                </div>
              </div>
              {discounted && (
                <p className="text-[10px] text-emerald-400 mt-2 pt-2 border-t border-white/5 light:border-slate-200">
                  Risparmi {euro(option.discount_cents)} pagando tutto subito. Lo sconto è sul prezzo del contratto: l&apos;IVA,
                  se dovuta, è calcolata sul prezzo scontato.
                </p>
              )}
