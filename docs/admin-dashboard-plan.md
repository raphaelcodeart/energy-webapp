...
| 4 | Scheda promoter estesa (dati fiscali, coordinate pagamento, tab contratti/provvigioni) | Parziale (Sessione 11-13): la vista "La mia Azienda" copre contratti/provvigioni per persona e per livello, con grafico e drill-down per livello. Sessione 13: scheda promoter ora editabile lato admin (nome, foto, qualifica, stato) -- non lo era affatto prima. Mancano ancora: dati fiscali/pagamento del promoter stesso |
| 5 | Vista rete enterprise (ricerca globale, navigazione bidirezionale, aggregati per nodo) | Parziale: vista admin org-wide con ricerca esiste (Sessione 10); aggregati per nodo esistono lato promoter (`branch-summary`, Sessione 11) ed **esposti anche lato admin** come widget "livelli e persone" whole-company (Sessione 12, `GET /network/organization/levels`). Sessione 13: bug reale corretto nella navigazione dell'albero (vedi `database-model.md §2`) |
